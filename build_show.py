...
Frames come from the picks group in _work/arrangement_kw.json, in his sequence.
...
def build():
    ...
    order = by_name.get(PICKS_GROUP, [])
    files = [by_num[n] for n in order if n in by_num and n not in aside]
    html = (PAGE.replace("__FRAMES__", json.dumps(files))
                .replace("__N__", str(len(files))))
    open(OUT, "w").write(html)
    print(f"wrote {OUT}  ({len(files)} frames, in his order)")
...
<div class=open id=open>
  ...
  <p class=hint>__N__ photographs, about four minutes. Best full screen with the sound off.</p>
</div>
...
<div id=stage>
  ...
  <div class=end><p style="color:#F3F1EC;font-size:17px">That is the summer.</p>
