...
def build(mode):
    ...
    if mode == "client":
        # The camp's own arranging tool: only delivered frames, one empty lane
        # of theirs, no internal state, export goes to an email they send.
        a = json.load(open(SAVED)) if os.path.exists(SAVED) else {}
        aside = set(a.get("aside", []))
        pool = [n for n in allf if n not in aside]
        ...
        html = (PAGE.replace("__SRC__", json.dumps(src))
                    ...
                    .replace("__KEY__", KEY + "_client")
                    .replace("__N__", str(len(pool)))
                    .replace("__FROM__", "a blank board")
                    .replace("Copy the arrangement", "Copy our arrangement")
                    .replace("Arrange the Kingswood set", "Arrange your Kingswood set"))
        # the template never closes body; the footer appends
        html = html + '''<div style="position:sticky;bottom:0;background:#0C3A54;border-top:1px solid rgba(243,241,236,.2);padding:10px 16px;font-size:13px;color:#F3F1EC;text-align:center">Group the frames any way you like, tap Copy our arrangement, and paste it into an email to <a style="color:#F0A882" href="mailto:noah@abba-photo.com?subject=Our%20Kingswood%20arrangement">noah@abba-photo.com</a>. It arrives exactly as you built it.</div>
</body></html>'''
        out = os.path.join(HERE, "arrange.html")
        open(out, "w").write(html)
        print(f"wrote {out} (client board: {len(pool)} frames, their lane only)")
        return
